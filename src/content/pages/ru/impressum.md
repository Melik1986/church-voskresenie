---
title: Impressum
description: Impressum (§ 5 DDG / § 18 MStV) — deutscher Rechtstext
locale: ru
section: legal
order: 1
---

# Impressum

Юридически значимый текст — на немецком языке (source of truth).

## Anbieter

**[PLACEHOLDER: Offizieller Name der Gemeinde]**

## Anschrift

Angerhauser Str. 91  
47259 Duisburg

## Vertretungsberechtigte Person

**[PLACEHOLDER: Vorname Nachname — Pastor / Vorstand]**

## Kontakt

E-Mail: **[PLACEHOLDER: E-Mail]**  
Telefon: **[PLACEHOLDER: Telefon]**

## Registereintrag (falls vorhanden)

Registergericht: **[PLACEHOLDER]**  
Registernummer: **[PLACEHOLDER: VR …]**

## Verantwortlicher i. S. d. § 18 Abs. 2 MStV

**[PLACEHOLDER: Name und Anschrift]**
