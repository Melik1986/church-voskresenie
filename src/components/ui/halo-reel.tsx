...
export interface HaloReelProps
  ...
  draggable?: boolean;
  /** Gap between neighbouring cards at the widest point, in card widths. @default 1.2 */
  spread?: number;
...
export function HaloReel({
  ...
  const slots = clamp(
    Math.ceil(
      TAU *
        Math.max(radiusX / (cardWidth * spread), radiusY / (cardHeight * spread)),
    ),
    count,
  ...
  return (
    ...
    >
      {item.src ? (
        ...
        <div
          className="flex h-full w-full flex-col items-center justify-center gap-1 bg-[var(--color-bg-soft)] p-3 text-center text-[var(--color-fg)]"
          style={{
        ...
        >
          {item.title ? (
            <span className="text-2xl font-black leading-none">{item.title}</span>
          ) : null}
          {item.subtitle ? (
